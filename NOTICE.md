...
| Rust native bridge             | `native/`                    | **PolyForm Perimeter 1.0.0**     | `typebit` 0.1.1 is now PolyForm Perimeter (not AGPL), so the statically-linked bridge can carry the same license. |
| Core engine                    | `typebit` 0.1.1 (external dependency) | **PolyForm Perimeter 1.0.0** | The `typebit` crate by blueokanna / HyphenTeam (per its Cargo.toml `license-file`). |
...
`typebit` 0.1.1 is PolyForm Perimeter — the same license as the Kotlin
application — the whole repository is uniformly PolyForm Perimeter 1.0.0.

## Third-party notices

- **typebit 0.1.1** — PolyForm Perimeter 1.0.0 — <https://github.com/blueokanna/TypeBit>
  ...
  UPnP/NAT-PMP, parallel piece verification, built-in rate limiting).
